...
[[nodiscard]]
inline jpegls_pc_parameters compute_default(const int32_t maximum_bit_sample_value, const int32_t near_lossless) noexcept
{
    ASSERT(maximum_bit_sample_value <= std::numeric_limits<uint16_t>::max());
    ASSERT(near_lossless >= 0 && near_lossless <= compute_maximum_near_lossless(maximum_bit_sample_value));
    ...
    if (maximum_bit_sample_value >= 128)
    {
        const int32_t factor{(std::min(maximum_bit_sample_value, 4095) + 128) / 256};
        const int threshold1{
            clamp(factor * (default_threshold1 - 2) + 2 + 3 * near_lossless, near_lossless + 1, maximum_bit_sample_value)};
        const int threshold2{
            clamp(factor * (default_threshold2 - 3) + 3 + 5 * near_lossless, threshold1, maximum_bit_sample_value)};

        return {maximum_bit_sample_value, threshold1, threshold2,
                clamp(factor * (default_threshold3 - 4) + 4 + 7 * near_lossless, threshold2, maximum_bit_sample_value),
                default_reset_threshold};
    }

    const int32_t factor{256 / (maximum_bit_sample_value + 1)};
    const int threshold1{
        clamp(std::max(2, default_threshold1 / factor + 3 * near_lossless), near_lossless + 1, maximum_bit_sample_value)};
    const int threshold2{
        clamp(std::max(3, default_threshold2 / factor + 5 * near_lossless), threshold1, maximum_bit_sample_value)};

    return {maximum_bit_sample_value, threshold1, threshold2,
            clamp(std::max(4, default_threshold3 / factor + 7 * near_lossless), threshold2, maximum_bit_sample_value),
            default_reset_threshold};
}
...
[[nodiscard]]
inline bool is_valid(const jpegls_pc_parameters& pc_parameters, const int32_t maximum_bit_sample_value,
                     const int32_t near_lossless, jpegls_pc_parameters* validated_parameters = nullptr) noexcept
{
    ASSERT(maximum_bit_sample_value >= 3 && maximum_bit_sample_value <= std::numeric_limits<uint16_t>::max());
    ...
    if (pc_parameters.maximum_sample_value != 0 &&
        (pc_parameters.maximum_sample_value < 1 || pc_parameters.maximum_sample_value > maximum_bit_sample_value))
        return false;

    const int32_t maximum_sample_value{pc_parameters.maximum_sample_value != 0 ? pc_parameters.maximum_sample_value
                                                                               : maximum_bit_sample_value};
