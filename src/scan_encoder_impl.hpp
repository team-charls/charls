...
class scan_encoder_impl final : public scan_encoder_core<sample_traits_t<Traits>>
{
    ...
    using base::encode_regular;
    using base::encode_run_interruption_component;
...
private:
    ...
    void encode_lines(const std::byte* source, const size_t stride)
    {
        ...
        for (uint32_t line{}; line < frame_info().height; ++line)
        {
            ...
            base::copy_source_to_line_buffer(source, current_line_ + 1, width_);
    ...
    FORCE_INLINE void encode_sample_line()
    {
        ...
        while (index <= width_)
        {
                ...
                qs != 0)
            {
                ...
                {
                    base::encode_regular_lossless(qs, current_line_[index], compute_predicted_value(ra, rb, rc));
                }
                else
                {
                    current_line_[index] =
                        encode_regular(qs, current_line_[index], compute_predicted_value(ra, rb, rc));
                }
                ++index;
            }
            else
    ...
    size_t encode_run_mode(const size_t start_index)
    {
        ...
        base::encode_run_pixels(run_length, run_length == count_type_remain);
    ...
    {
        return {encode_run_interruption_component(x.v1, ra.v1, rb.v1),
                encode_run_interruption_component(x.v2, ra.v2, rb.v2)};
    }
                                                       ...
                                                       const triplet<sample_type> rb)
    {
        return {encode_run_interruption_component(x.v1, ra.v1, rb.v1),
                encode_run_interruption_component(x.v2, ra.v2, rb.v2),
                encode_run_interruption_component(x.v3, ra.v3, rb.v3)};
    }
                                                    ...
                                                    const quad<sample_type> rb)
    {
        return {encode_run_interruption_component(x.v1, ra.v1, rb.v1),
                encode_run_interruption_component(x.v2, ra.v2, rb.v2),
                encode_run_interruption_component(x.v3, ra.v3, rb.v3),
                encode_run_interruption_component(x.v4, ra.v4, rb.v4)};
    }
