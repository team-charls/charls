...
void benchmark_decode(const path& filename, const uint32_t loop_count)
{
    ASSERT(loop_count > 0);
...
void benchmark_encode(const path& filename, const uint32_t loop_count)
{
    ASSERT(loop_count > 0);

    cout << "Test encode performance with loop count " << loop_count << " and " << filename << "\n";

    const support::portable_anymap_file anymap_file(filename);
