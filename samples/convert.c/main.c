...
static void *encode_bmp_to_jpegls(const void *pixel_data, size_t pixel_data_size, const bmp_dib_header_t *header, size_t *bytes_writen)
{
    ...
    CharlsApiResultType result = JpegLsEncode(encoded_buffer, encoded_buffer_size, bytes_writen, pixel_data, pixel_data_size, &info);
    if (result != CHARLS_API_RESULT_OK)
    {
        printf("Failed to encode pixel data: %i\n", result);
        free(encoded_buffer);
        encoded_buffer = NULL;
    }
