...
void convert_bgr_to_rgb(std::vector<std::byte>& triplet_buffer, const size_t width, const size_t height,
...
std::vector<std::byte> triplet_to_planar(const std::vector<std::byte>& buffer, const size_t width, const size_t height,
                                         const size_t stride)
{
    std::vector<std::byte> result(bytes_per_rgb_pixel * width * height);
...
void convert_bottom_up_to_top_down(std::byte* triplet_buffer, const size_t width, const size_t height, const size_t stride)
{
    ...
    for (size_t i{}; i != height / 2; ++i)
    {
        ...
        std::swap_ranges(triplet_buffer + (i * stride), triplet_buffer + (i * stride) + row_length,
                         triplet_buffer + (bottom_row * stride));
    }
}

std::vector<std::byte> encode_bmp_image_to_jpegls(const bmp_image& image, const charls::interleave_mode interleave_mode,
                                                  const int near_lossless)
{
    ...
    std::vector<std::byte> buffer(encoder.estimated_destination_size());
...
void save_buffer_to_file(const void* buffer, const size_t buffer_size, const std::filesystem::path& filename)
{
    assert(buffer);
    assert(buffer_size);
