...
void test_by_decoding(const vector<byte>& encoded_source, const frame_info& source_frame_info,
                      ...
                      const color_transformation color_transformation = color_transformation::none)
{
    ...
    EXPECT_EQ(source_frame_info.width, frame_info.width);
    EXPECT_EQ(source_frame_info.height, frame_info.height);
    EXPECT_EQ(source_frame_info.bits_per_sample, frame_info.bits_per_sample);
    EXPECT_EQ(source_frame_info.component_count, frame_info.component_count);
    ASSERT_TRUE(interleave_mode == decoder.get_interleave_mode());
    ASSERT_TRUE(color_transformation == decoder.color_transformation());

    vector<byte> destination(decoder.get_destination_size());
    decoder.decode(destination);

    EXPECT_EQ(destination.size(), expected_destination_size);
...
void encode_with_custom_preset_coding_parameters(const jpegls_pc_parameters& pc_parameters)
{
...
[[nodiscard]]
vector<byte>::const_iterator find_first_lse_segment(const vector<byte>::const_iterator begin,
...
TEST(jpegls_encoder_test, create_destroy)
{
    // ReSharper disable once CppLocalVariableWithNonTrivialDtorIsNeverUsed
    jpegls_encoder encoder;
}

TEST(jpegls_encoder_test, create_and_move)
{
...
TEST(jpegls_encoder_test, frame_info_max_and_min)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, frame_info_bad_bits_per_sample_throws)
{
    ...
    assert_expect_exception(jpegls_errc::invalid_argument_bits_per_sample, [&encoder] { encoder.frame_info({1, 1, 1, 1}); });
    assert_expect_exception(jpegls_errc::invalid_argument_bits_per_sample,
                            [&encoder] { encoder.frame_info({1, 1, 17, 1}); });
}

TEST(jpegls_encoder_test, frame_info_bad_component_count_throws)
{
    jpegls_encoder encoder;

    assert_expect_exception(jpegls_errc::invalid_argument_component_count, [&encoder] { encoder.frame_info({1, 1, 2, 0}); });
    assert_expect_exception(jpegls_errc::invalid_argument_component_count,
                            [&encoder] { encoder.frame_info({1, 1, 2, 256}); });
}

TEST(jpegls_encoder_test, interleave_mode)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, interleave_mode_bad_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, interleave_mode_does_not_match_component_count_throws)
{
    constexpr frame_info frame_info{512, 512, 8, 1};
    vector<byte> source(static_cast<size_t>(frame_info.width) * frame_info.height);

    assert_expect_exception(jpegls_errc::invalid_argument_interleave_mode,
                            [&frame_info, &source] { jpegls_encoder::encode(source, frame_info, interleave_mode::sample); });
    assert_expect_exception(jpegls_errc::invalid_argument_interleave_mode,
                            [&frame_info, &source] { jpegls_encoder::encode(source, frame_info, interleave_mode::line); });
}

TEST(jpegls_encoder_test, near_lossless)
{
    jpegls_encoder encoder;
    ...
    ASSERT_TRUE(true); // No exception should be thrown.
}

TEST(jpegls_encoder_test, near_lossless_out_of_range_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, near_lossless_just_in_range_compared_to_max_bit_sample_value)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(destination[0x19], byte{0x80});
}

TEST(jpegls_encoder_test, near_lossless_out_of_range_compared_to_max_bit_sample_value_throws)
{
    jpegls_encoder encoder;
    encoder.frame_info({1, 1, 2, 1});
    std::vector<byte> destination(encoder.estimated_destination_size());
    ...
    const std::vector<byte> pixels(1);
    assert_expect_exception(jpegls_errc::invalid_argument_near_lossless, [&encoder, &pixels] { encoder.encode(pixels); });
}

TEST(jpegls_encoder_test, near_lossless_just_in_range_compared_to_max_sample_value)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(destination[0x28], byte{0x80});
}

TEST(jpegls_encoder_test, near_lossless_out_of_range_compared_to_max_sample_value_throws)
{
    jpegls_encoder encoder;
    encoder.frame_info({1, 1, 8, 1});
    std::vector<byte> destination(encoder.estimated_destination_size());
...
TEST(jpegls_encoder_test, estimated_destination_size_minimal_frame_info)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, estimated_destination_size_maximal_frame_info)
{
    jpegls_encoder encoder;

    encoder.frame_info({numeric_limits<uint16_t>::max(), numeric_limits<uint16_t>::max(), 8, 1}); // = maximum.
    const auto size{encoder.estimated_destination_size()};
    constexpr auto expected{static_cast<size_t>(numeric_limits<uint16_t>::max()) * numeric_limits<uint16_t>::max() * 1 * 1};
    ASSERT_TRUE(size >= expected);
}

TEST(jpegls_encoder_test, estimated_destination_size_monochrome_16_bit)
{
    jpegls_encoder encoder;

    encoder.frame_info({100, 100, 16, 1}); // minimum.
    const auto size{encoder.estimated_destination_size()};
    ASSERT_TRUE(size >= size_t{100} * 100 * 2);
}

TEST(jpegls_encoder_test, estimated_destination_size_color_8_bit)
{
    jpegls_encoder encoder;

    encoder.frame_info({2000, 2000, 8, 3});
    const auto size{encoder.estimated_destination_size()};
    ASSERT_TRUE(size >= size_t{2000} * 2000 * 3);
}

TEST(jpegls_encoder_test, estimated_destination_size_very_wide)
{
    jpegls_encoder encoder;

    encoder.frame_info({numeric_limits<uint16_t>::max(), 1, 8, 1});
    const auto size{encoder.estimated_destination_size()};
    ASSERT_TRUE(size >= static_cast<size_t>(numeric_limits<uint16_t>::max()) + 1024U);
}

TEST(jpegls_encoder_test, estimated_destination_size_very_high)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, estimated_destination_size_too_soon_throws)
{
    const jpegls_encoder encoder;

    assert_expect_exception(jpegls_errc::invalid_operation, [&encoder] { ignore = encoder.estimated_destination_size(); });
}

TEST(jpegls_encoder_test, estimated_destination_size_thath_causes_overflow_throws)
{
    jpegls_encoder encoder;
    ...
    const auto size{encoder.estimated_destination_size()};
    ASSERT_TRUE(size != 0); // actual value already checked in other test functions.
...
TEST(jpegls_encoder_test, destination_can_be_set_multiple_times_before_writing)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(serialized_spiff_header_size + 2, encoder.bytes_written());
}

TEST(jpegls_encoder_test, destination_can_not_be_set_after_writing_throws)
{
    jpegls_encoder encoder;
    ...
    assert_expect_exception(jpegls_errc::invalid_operation, [&encoder, &destination] { encoder.destination(destination); });
}

TEST(jpegls_encoder_test, write_standard_spiff_header)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(byte{0xFF}, destination[0]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::start_of_image), destination[1]);

    // Verify that a APP8 with SPIFF has been written (details already verified by jpeg_stream_writer_test).
    EXPECT_EQ(byte{0xFF}, destination[2]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::application_data8), destination[3]);
    EXPECT_EQ(byte{}, destination[4]);
    EXPECT_EQ(byte{32}, destination[5]);
    EXPECT_EQ(byte{'S'}, destination[6]);
    EXPECT_EQ(byte{'P'}, destination[7]);
    EXPECT_EQ(byte{'I'}, destination[8]);
    EXPECT_EQ(byte{'F'}, destination[9]);
    EXPECT_EQ(byte{'F'}, destination[10]);
    EXPECT_EQ(byte{}, destination[11]);
}

TEST(jpegls_encoder_test, write_standard_spiff_header_with_non_matching_color_space)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_standard_spiff_header_without_destination_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_standard_spiff_header_without_frame_info_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_standard_spiff_header_twice_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_spiff_header)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_spiff_header_invalid_height_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_spiff_entry)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(size_t{48}, encoder.bytes_written());
}

TEST(jpegls_encoder_test, write_spiff_entry_twice)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(size_t{60}, encoder.bytes_written());
}

TEST(jpegls_encoder_test, write_empty_spiff_entry)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(size_t{44}, encoder.bytes_written());
}

TEST(jpegls_encoder_test, write_spiff_entry_with_invalid_tag_throws)
{
    jpegls_encoder encoder;
    ...
    assert_expect_exception(jpegls_errc::invalid_argument, [&encoder] {
        constexpr int32_t spiff_end_of_directory_entry_type_value{1};
        encoder.write_spiff_entry(spiff_end_of_directory_entry_type_value, "test", 4);
    });
}

TEST(jpegls_encoder_test, write_spiff_entry_with_invalid_size_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_spiff_entry_without_spiff_header_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_spiff_end_of_directory_entry)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(byte{0xFF}, destination[44]);
    EXPECT_EQ(byte{0xD8}, destination[45]); // 0xD8 = SOI: Marks the start of an image.
}

TEST(jpegls_encoder_test, write_spiff_end_of_directory_entry_before_header_throws)
{
    jpegls_encoder encoder;
    ...
    assert_expect_exception(jpegls_errc::invalid_operation, [&encoder] { encoder.write_spiff_end_of_directory_entry(); });
}

TEST(jpegls_encoder_test, write_spiff_end_of_directory_entry_twice_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_comment)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(size_t{10}, encoder.bytes_written());
    ...
    EXPECT_EQ(byte{0xFF}, destination[2]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::comment), destination[3]);
    EXPECT_EQ(byte{}, destination[4]);
    EXPECT_EQ(byte{2 + 4}, destination[5]);
    EXPECT_EQ(byte{'1'}, destination[6]);
    EXPECT_EQ(byte{'2'}, destination[7]);
    EXPECT_EQ(byte{'3'}, destination[8]);
    EXPECT_EQ(byte{}, destination[9]);
}

TEST(jpegls_encoder_test, write_empty_comment)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(size_t{6}, encoder.bytes_written());
    ...
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::comment), destination[3]);
    EXPECT_EQ(byte{}, destination[4]);
    EXPECT_EQ(byte{2}, destination[5]);
}

TEST(jpegls_encoder_test, write_empty_comment_buffer)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_max_comment)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(destination.size(), encoder.bytes_written());
    ...
    EXPECT_EQ(byte{0xFF}, destination[2]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::comment), destination[3]);
    EXPECT_EQ(byte{255}, destination[4]);
    EXPECT_EQ(byte{255}, destination[5]);
}

TEST(jpegls_encoder_test, write_two_comment)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(byte{0xFF}, destination[10]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::comment), destination[11]);
    EXPECT_EQ(byte{}, destination[12]);
    EXPECT_EQ(byte{2}, destination[13]);
}

TEST(jpegls_encoder_test, write_too_large_comment_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_comment_null_pointer_with_size_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_comment_after_encode_throws)
{
...
TEST(jpegls_encoder_test, write_comment_before_encode)
{
    const vector source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
...
TEST(jpegls_encoder_test, write_application_data)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(byte{0xFF}, destination[2]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::application_data1), destination[3]);
    EXPECT_EQ(byte{}, destination[4]);
    EXPECT_EQ(byte{2 + 4}, destination[5]);
    EXPECT_EQ(byte{1}, destination[6]);
    EXPECT_EQ(byte{2}, destination[7]);
    EXPECT_EQ(byte{3}, destination[8]);
    EXPECT_EQ(byte{4}, destination[9]);
}

TEST(jpegls_encoder_test, write_empty_application_data)
{
    jpegls_encoder encoder;
    ...
    // Verify that a APPn segment has been written.
    EXPECT_EQ(byte{0xFF}, destination[2]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::application_data2), destination[3]);
    EXPECT_EQ(byte{}, destination[4]);
    EXPECT_EQ(byte{2}, destination[5]);
}

TEST(jpegls_encoder_test, write_max_application_data)
{
    jpegls_encoder encoder;
    ...
    // Verify that a APPn segment has been written.
    EXPECT_EQ(byte{0xFF}, destination[2]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::application_data15), destination[3]);
    EXPECT_EQ(byte{255}, destination[4]);
    EXPECT_EQ(byte{255}, destination[5]);
}

TEST(jpegls_encoder_test, write_two_application_data)
{
    jpegls_encoder encoder;
    ...
    // Verify that the COM segments have been written.
    EXPECT_EQ(byte{0xFF}, destination[2]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::application_data0), destination[3]);
    EXPECT_EQ(byte{}, destination[4]);
    EXPECT_EQ(byte{2 + 4}, destination[5]);
    EXPECT_EQ(byte{1}, destination[6]);
    EXPECT_EQ(byte{2}, destination[7]);
    ...
    EXPECT_EQ(byte{0xFF}, destination[10]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::application_data8), destination[11]);
    EXPECT_EQ(byte{}, destination[12]);
    EXPECT_EQ(byte{2}, destination[13]);
}

TEST(jpegls_encoder_test, write_too_large_application_data_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_application_data_null_pointer_with_size_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_application_data_after_encode_throws)
{
    const vector source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
...
TEST(jpegls_encoder_test, write_application_data_with_bad_id_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_application_data_before_encode)
{
...
TEST(jpegls_encoder_test, write_mapping_table)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(byte{0xFF}, destination[2]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::jpegls_preset_parameters), destination[3]);
    EXPECT_EQ(byte{}, destination[4]);
    EXPECT_EQ(byte{6}, destination[5]);
    EXPECT_EQ(byte{2}, destination[6]);
    EXPECT_EQ(byte{1}, destination[7]);
    EXPECT_EQ(byte{1}, destination[8]);
    EXPECT_EQ(byte{}, destination[9]);
}

TEST(jpegls_encoder_test, write_mapping_table_before_encode)
{
...
TEST(jpegls_encoder_test, write_mapping_table_with_bad_table_id_throws)
{
    constexpr array table_data{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_mapping_table_with_bad_entry_size_throws)
{
    constexpr array table_data{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_mapping_table_with_too_small_table_throws)
{
    constexpr array table_data{byte{0}};
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_mapping_table_null_pointer_with_size_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, write_mapping_table_after_encode_throws)
{
    constexpr array table_data{byte{0}};
    const vector source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
...
TEST(jpegls_encoder_test, create_abbreviated_format)
{
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(size_t{12}, bytes_written);
    ...
    // Check that SOI marker has been written.
    EXPECT_EQ(byte{0xFF}, destination[10]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::end_of_image), destination[11]);
}

TEST(jpegls_encoder_test, create_abbreviated_format_with_no_tables_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, set_preset_coding_parameters)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, set_preset_coding_parameters_bad_values_throws)
{
    constexpr array source{byte{0}, byte{1}, byte{1}, byte{1}, byte{0}};
    constexpr frame_info frame_info{5, 1, 8, 1};
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, set_preset_coding_parameters_bad_maximum_sample_value_throws)
{
    constexpr array source{byte{0}, byte{1}, byte{1}, byte{1}, byte{0}};
    constexpr frame_info frame_info{5, 1, 8, 1};
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, encode_with_preset_coding_parameters_non_default_values)
{
...
TEST(jpegls_encoder_test, set_color_transformation_bad_value_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, set_table_id)
{
    constexpr array source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
    constexpr frame_info frame_info{3, 1, 16, 1};
    jpegls_encoder encoder;
    ...
    EXPECT_EQ(1, decoder.get_mapping_table_id(0));
}

TEST(jpegls_encoder_test, set_table_id_clear_id)
{
    constexpr array source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
    constexpr frame_info frame_info{3, 1, 16, 1};
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, set_table_id_bad_component_index_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, set_table_id_bad_id_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, encode_without_destination_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, encode_without_frame_info_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, encode_with_spiff_header)
{
...
TEST(jpegls_encoder_test, encode_with_color_transformation)
{
    constexpr array source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
...
TEST(jpegls_encoder_test, encode_16_bit)
{
    constexpr array source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
    constexpr frame_info frame_info{3, 1, 16, 1};
...
TEST(jpegls_encoder_test, simple_encode)
{
    const vector source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
...
TEST(jpegls_encoder_test, encode_with_stride_interleave_none_8_bit)
{
...
TEST(jpegls_encoder_test, encode_with_stride_interleave_none_8_bit_small_image)
{
...
TEST(jpegls_encoder_test, encode_with_stride_interleave_none_16_bit)
{
    ...
    test_by_decoding(destination, frame_info, expected_destination.data(), expected_destination.size() * sizeof(uint16_t),
                     interleave_mode::none);
}

TEST(jpegls_encoder_test, encode_with_stride_interleave_sample_8_bit)
{
...
TEST(jpegls_encoder_test, encode_with_stride_interleave_sample_16_bit)
{
...
TEST(jpegls_encoder_test, encode_with_bad_stride_interleave_none_throws)
{
    ...
    assert_expect_exception(jpegls_errc::invalid_argument_size, [&encoder, &source] { ignore = encoder.encode(source, 4); });
}

TEST(jpegls_encoder_test, encode_with_bad_stride_interleave_sample_throws)
{
    ...
    assert_expect_exception(jpegls_errc::invalid_argument_size, [&encoder, &source] { ignore = encoder.encode(source, 7); });
}

TEST(jpegls_encoder_test, encode_with_too_small_stride_interleave_none_throws)
{
...
TEST(jpegls_encoder_test, encode_with_too_small_stride_interleave_sample_throws)
{
    constexpr array<byte, 12> source{byte{100}, byte{150}, byte{200}, byte{100}, byte{150},
                                     byte{200}, byte{100}, byte{150}, byte{200}};
    constexpr frame_info frame_info{2, 1, 8, 3};
...
TEST(jpegls_encoder_test, encode_1_component_4_bit_with_high_bits_set)
{
...
TEST(jpegls_encoder_test, encode_1_component_12_bit_with_high_bits_set)
{
...
TEST(jpegls_encoder_test, encode_3_components_6_bit_with_high_bits_set_interleave_mode_sample)
{
...
TEST(jpegls_encoder_test, encode_3_components_6_bit_with_high_bits_set_interleave_mode_line)
{
    const vector source(size_t{512} * 512 * 3, byte{0xFF});
    constexpr frame_info frame_info{512, 512, 6, 3};
...
TEST(jpegls_encoder_test, encode_3_components_10_bit_with_high_bits_set_interleave_mode_sample)
{
...
TEST(jpegls_encoder_test, encode_3_components_10_bit_with_high_bits_set_interleave_mode_line)
{
    const vector source(size_t{512} * 512 * 2 * 3, byte{0xFF});
    constexpr frame_info frame_info{512, 512, 10, 3};
...
TEST(jpegls_encoder_test, encode_4_components_5_bit_with_high_bits_set_interleave_mode_line)
{
...
TEST(jpegls_encoder_test, encode_4_components_7_bit_with_high_bits_set_interleave_mode_sample)
{
    const vector source(size_t{512} * 512 * 4, byte{0xFF});
...
TEST(jpegls_encoder_test, encode_4_components_11_bit_with_high_bits_set_interleave_mode_line)
{
...
TEST(jpegls_encoder_test, encode_4_components_13_bit_with_high_bits_set_interleave_mode_sample)
{
    const vector source(size_t{512} * 512 * 2 * 4, byte{0xFF});
...
TEST(jpegls_encoder_test, rewind)
{
    constexpr array source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
    constexpr frame_info frame_info{3, 1, 16, 1};
    ...
    EXPECT_EQ(bytes_written1, bytes_written2);
    ASSERT_TRUE(destination_backup == destination);
}

TEST(jpegls_encoder_test, rewind_before_destination)
{
    constexpr array source{byte{0}, byte{1}, byte{2}, byte{3}, byte{4}, byte{5}};
    constexpr frame_info frame_info{3, 1, 16, 1};
...
TEST(jpegls_encoder_test, encode_image_odd_size)
{
    constexpr frame_info frame_info{512, 512, 8, 1};
    ...
    EXPECT_EQ(size_t{99}, destination.size());
    test_by_decoding(destination, frame_info, source.data(), source.size(), interleave_mode::none);
}

TEST(jpegls_encoder_test, encode_image_odd_size_forced_even)
{
    constexpr frame_info frame_info{512, 512, 8, 1};
    const vector<byte> source(static_cast<size_t>(frame_info.width) * frame_info.height);
    ...
    EXPECT_EQ(size_t{100}, destination.size());
    test_by_decoding(destination, frame_info, source.data(), source.size(), interleave_mode::none);
}

TEST(jpegls_encoder_test, encode_image_forced_version_comment)
{
    constexpr frame_info frame_info{512, 512, 8, 1};
    const vector<byte> source(static_cast<size_t>(frame_info.width) * frame_info.height);
    ...
    EXPECT_EQ(expected.size() + 1, actual_size);
    ASSERT_TRUE(memcmp(expected.data(), actual_data, actual_size) == 0);
}

TEST(jpegls_encoder_test, encode_image_include_pc_parameters_jai)
{
    ...
    EXPECT_EQ(size_t{43}, bytes_written);

    EXPECT_EQ(byte{0xFF}, destination[15]);
    EXPECT_EQ(static_cast<byte>(jpeg_marker_code::jpegls_preset_parameters), destination[16]);

    // Segment size.
    EXPECT_EQ(byte{}, destination[17]);
    EXPECT_EQ(byte{13}, destination[18]);

    // Parameter ID.
    EXPECT_EQ(byte{0x1}, destination[19]);

    // MaximumSampleValue
    EXPECT_EQ(byte{255}, destination[20]);
    EXPECT_EQ(byte{255}, destination[21]);

    constexpr thresholds expected{compute_defaults_using_reference_implementation(std::numeric_limits<uint16_t>::max(), 0)};

    const int32_t threshold1{to_integer<int32_t>(destination[22]) << 8 | to_integer<int32_t>(destination[23])};
    EXPECT_EQ(expected.t1, threshold1);

    const int32_t threshold2{to_integer<int32_t>(destination[24]) << 8 | to_integer<int32_t>(destination[25])};
    EXPECT_EQ(expected.t2, threshold2);

    const int32_t threshold3{to_integer<int32_t>(destination[26]) << 8 | to_integer<int32_t>(destination[27])};
    EXPECT_EQ(expected.t3, threshold3);

    const int32_t reset{to_integer<int32_t>(destination[28] << 8) | to_integer<int32_t>(destination[29])};
    EXPECT_EQ(expected.reset, reset);
}

TEST(jpegls_encoder_test, encode_image_with_include_pc_parameters_jai_not_set)
{
    constexpr frame_info frame_info{1, 1, 16, 1};
    const vector<uint16_t> source(static_cast<size_t>(frame_info.width) * frame_info.height);
    ...
    EXPECT_EQ(size_t{28}, bytes_written);
}

TEST(jpegls_encoder_test, set_invalid_encode_options_throws)
{
    jpegls_encoder encoder;
...
TEST(jpegls_encoder_test, large_image_contains_lse_for_oversize_image_dimension)
{
    ...
    const size_t bytes_written{encoder.encode(source)};
    EXPECT_EQ(size_t{46}, bytes_written);

    destination.resize(bytes_written);
    const auto it{find_first_lse_segment(destination.cbegin(), destination.cend())};
    ASSERT_TRUE(it != destination.cend());
}

TEST(jpegls_encoder_test, encode_oversized_image)
{
...
TEST(jpegls_encoder_test, image_contains_no_preset_coding_parameters_by_default)
{
    constexpr frame_info frame_info{512, 512, 8, 1};
    const vector<byte> source(static_cast<size_t>(frame_info.width) * frame_info.height);
    ...
    const size_t bytes_written{encoder.encode(source)};
    EXPECT_EQ(size_t{99}, bytes_written);
...
TEST(jpegls_encoder_test, image_contains_no_preset_coding_parameters_if_configured_pc_is_default)
{
    constexpr frame_info frame_info{512, 512, 8, 1};
    const vector<byte> source(static_cast<size_t>(frame_info.width) * frame_info.height);
...
TEST(jpegls_encoder_test, image_contains_preset_coding_parameters_if_configured_pc_is_non_default)
{
    constexpr frame_info frame_info{512, 512, 8, 1};
    const vector<byte> source(static_cast<size_t>(frame_info.width) * frame_info.height);
    ...
    const size_t bytes_written{encoder.encode(source)};
    EXPECT_EQ(size_t{114}, bytes_written);

    destination.resize(bytes_written);
    const auto it{find_first_lse_segment(destination.cbegin(), destination.cend())};
    ASSERT_FALSE(it == destination.cend());
}

TEST(jpegls_encoder_test, image_contains_preset_coding_parameters_if_configured_pc_has_diff_max_value)
{
    constexpr frame_info frame_info{512, 512, 8, 1};
    const vector<byte> source(static_cast<size_t>(frame_info.width) * frame_info.height);
...
TEST(jpegls_encoder_test, encode_to_buffer_with_uint16_size_works)
{
